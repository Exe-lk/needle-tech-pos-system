...
interface AutoGeneratedInvoice {
  ...
  periodFrom: string;
  periodTo: string;
  items: (AutoGeneratedInvoiceItem | AutoGeneratedInvoiceToolItem)[];
  subtotal: number;
  vatAmount: number;
  totalAmount: number;
  status: 'issued';
...
interface AutoGeneratedInvoiceToolItem {
  id: string;
  itemCode: string;
  description: string;
  quantity: number;
  ...
  const handleAgreementUpdate = async (data: Record<string, unknown>) => {
    ...
    try {
      ...
      if (wasPending && totalExpected > 0 && addedCount >= totalExpected) {
        ...
        const toolsMonthlySubtotalRaw = toolsForAgreement.reduce((sum, t) => {
          const qty = typeof t.quantity === 'number' ? t.quantity : 0;
          const unit = typeof t.unitPrice === 'number' ? t.unitPrice : 0;
          return sum + qty * unit;
        }, 0);
        const toolsMonthlySubtotal = Math.round(toolsMonthlySubtotalRaw * 100) / 100;
        ...
        // Fetch the rental detail to read per-assigned-machine `dailyRate` from rental_machine.
        // Business rule: invoice line item unitPrice (monthly) = dailyRate * 30.
        const rentalDetailRes = await authFetch(`/api/v1/rentals/${rentalId}`, { method: 'GET' });
        const rentalDetailJson = await rentalDetailRes.json();
        if (!rentalDetailRes.ok) {
          const msg =
            rentalDetailJson?.message ??
            rentalDetailJson?.errors ??
            'Failed to load rental detail for invoice rates.';
          alert(typeof msg === 'string' ? msg : JSON.stringify(msg));
          return;
        }
        const rentalDetail = rentalDetailJson?.data;
        const rentalMachines: any[] = Array.isArray(rentalDetail?.machines) ? rentalDetail.machines : [];
        const dailyRateBySerial = new Map<string, number>();
        rentalMachines.forEach((rm: any) => {
          const serial = String(rm?.machine?.serialNumber ?? '').trim();
          if (!serial) return;
          const n = Number(rm?.dailyRate);
          dailyRateBySerial.set(serial, Number.isFinite(n) ? n : 0);
        });
        ...
        // Build invoice items grouped by category + dailyRate, so differing machine rates generate distinct lines.
        ...
        machinesForAgreement.forEach((m) => {
          ...
          const dailyRate = dailyRateBySerial.get(serial) ?? 0;
          const monthlyRentPerMachine = (Number.isFinite(dailyRate) ? dailyRate : 0) * 30;
          const desc = [cat.brand, cat.model, cat.type].filter(Boolean).join(' ') || 'Machine';
          const key = `${norm(cat.brand)}||${norm(cat.model)}||${norm(cat.type)}||${monthlyRentPerMachine}`;

          const existing = invoiceItemGroups.get(key);
          if (existing) {
            existing.numberOfMachines += 1;
          } else {
            invoiceItemGroups.set(key, {
              brand: cat.brand,
              model: cat.model,
              type: cat.type,
        ...
        const invoiceItems: AutoGeneratedInvoiceItem[] = Array.from(invoiceItemGroups.values()).map((g, index) => {
          const lineSubtotal = g.numberOfMachines * g.monthlyRentPerMachine;
          return {
            id: String(index + 1),
            itemCode: `212WG${String(index + 1).padStart(5, '0')}`,
            description: g.description,
            brand: g.brand,
            model: g.model,
            type: g.type,
            numberOfMachines: g.numberOfMachines,
            monthlyRentPerMachine: g.monthlyRentPerMachine,
            subtotal: lineSubtotal,
          };
        });

        const toolInvoiceItems: AutoGeneratedInvoiceToolItem[] = toolsForAgreement
          .filter((t) => (t.quantity ?? 0) > 0 && (t.unitPrice ?? 0) >= 0)
          .map((t, idx) => {
            const toolName = (t.tool?.name ?? 'Tool').trim() || 'Tool';
            const toolType = (t.tool?.type ?? '').trim();
            const desc = [toolName, toolType].filter(Boolean).join(' - ');
            const qty = t.quantity ?? 0;
            const unitPrice = t.unitPrice ?? 0;
            const subtotal = qty * unitPrice;
            return {
              id: String(invoiceItems.length + idx + 1),
              itemCode: `212TL${String(idx + 1).padStart(5, '0')}`,
              description: desc.toUpperCase(),
              quantity: qty,
              unitPrice,
              subtotal,
        ...
        // Clear calculation: subtotal = sum of line subtotals, VAT, grandTotal = subtotal + vatAmount
        const subtotalRaw =
          invoiceItems.reduce((s, it) => s + it.subtotal, 0) +
          toolInvoiceItems.reduce((s, it) => s + it.subtotal, 0);
        const subtotal = Math.round(subtotalRaw * 100) / 100;
        const vatAmount = Math.round(subtotal * VAT_RATE * 100) / 100;
        const totalAmount = Math.round((subtotal + vatAmount) * 100) / 100;

        // Build line items in API shape (matches invoice page)
        const machineLineItemsForApi = invoiceItems.map((item) => ({
        ...
        const toolLineItemsForApi = toolInvoiceItems.map((item) => ({
          description: item.description,
        ...
        const lineItemsForApi = [...machineLineItemsForApi, ...toolLineItemsForApi];
        ...
        const generatedInvoice: AutoGeneratedInvoice = {
          ...
          periodFrom,
          periodTo,
          items: [...invoiceItems, ...toolInvoiceItems],
          subtotal,
          vatAmount,
          totalAmount,
          status: 'issued',
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800/40 border border-gray-200 dark:border-slate-700/60 rounded-2xl p-5 space-y-3 shadow-sm dark:shadow-none">
              <div className="flex items-center justify-between gap-2">
                ...
                <p className="text-xs text-gray-500 dark:text-slate-400">
                  {toolsForAgreement.length === 0
                    ? 'No tools'
                    : `Rs. ${Math.round(toolsMonthlySubtotal * 100) / 100}`}{toolsForAgreement.length === 0 ? '' : ' / month'}
                </p>
              </div>
              {toolsForAgreement.length === 0 ? (
                <p className="text-xs text-gray-500 dark:text-slate-500">
                  No tools are linked to this agreement.
                </p>
              ) : (
                <div className="space-y-2">
                  {toolsForAgreement.map((t, idx) => {
                    const name = (t.tool?.name ?? 'Tool').toString().trim() || 'Tool';
                    const type = (t.tool?.type ?? '').toString().trim();
                    const brandModel = [t.tool?.brand, t.tool?.model].filter(Boolean).join(' ');
                    const serial = (t.tool?.serialNumber ?? '').toString().trim();
                    return (
                      <div
                        key={t.id ?? `${t.toolId ?? 'tool'}-${idx}`}
                        className="bg-gray-50 dark:bg-slate-900/40 border border-gray-200 dark:border-slate-700/60 rounded-xl px-4 py-3"
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-500 dark:text-slate-400">
                              {[
                                brandModel ? `Model: ${brandModel}` : null,
                          ...
                          <div className="text-right text-xs shrink-0">
                            <p className="text-gray-500 dark:text-slate-400">Qty</p>
                            <p className="font-semibold text-gray-900 dark:text-slate-100">
                              {t.quantity}
                            </p>
                          </div>
                        ...
                        <div className="mt-2 flex items-center justify-between text-xs">
                          <p className="text-gray-500 dark:text-slate-400">
                            Unit: Rs. {(t.unitPrice ?? 0).toLocaleString('en-LK')}
                          </p>
                          <p className="font-semibold text-gray-900 dark:text-slate-100">
                            Line: Rs. {((t.unitPrice ?? 0) * (t.quantity ?? 0)).toLocaleString('en-LK')}
                          </p>
                        </div>
                      </div>
                    );
